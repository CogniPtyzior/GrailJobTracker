...
/*
 * Doctrine adapter for the user repository port.
 * It preserves the existing schema and query semantics while returning domain users to application code.
 */
...
final class DoctrineUserRepository extends ServiceEntityRepository implements UserRepositoryInterface
{
    ...
    public function search(?bool $isActive, ?string $query, int $page, int $pageSize): array
    {
        ...
        if ($query !== null && trim($query) !== '') {
            ...
            $qb
                ->andWhere('LOWER(u.email) LIKE :term OR '.$this->nameSearchExpression().' LIKE :term')
    ...
    {
        $record = $this->find($user->getId()->toRfc4122()) ?? new UserRecord();
    ...
    {
        $record = $this->find($user->getId()->toRfc4122());
    ...
    {
        return "LOWER(CONCAT(COALESCE(u.firstName, ''), ' ', COALESCE(u.lastName, '')))";
    }
}
